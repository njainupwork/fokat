...
const NFTCard: React.FC = () => {
  ...
  const { getUserTokens, enterGame, approveNFT, transferToCompany } =
    useCharacter();
  ...
  useEffect(() => {
    ...
    getUserTokens().then((tokens) => {
      ...
      Promise.all(tokens).then((tokens) => {
        console.log("🚀 ~ file: NFTCard.tsx ~ line 275 ~ Promise.all ~ results", tokens)
  ...
  const selectCharacter = (token: number) => {
    transferToCompany(token, "0xCc99855481022fCc44037DC50b48e3D7415613AD").then(
      (transaction) => {
        console.log(
          "🚀 ~ file: NFTCard.tsx ~ line 287 ~ selectCharacter ~ transaction",
          transaction
        );
  ...
  console.log("🚀 ~ file: NFTCard.tsx ~ line 316 ~ tokens", tokens)
